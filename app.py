...
@app.route("/duonut_test", methods = ["POST", "GET"])
def duonut_test():
    translated = ""
    dict_json = ""
    # url = "https://freekode.centeltech.com/api/minions?txt={}".format(text)
    url = 'https://backend.duonut.com/api/v1/duonut/viewer/861376'
    response = requests.get(url).text
    dict_json = json.loads(response)
    translated = dict_json
    return render_template("duonut_test.html", translated = translated)
